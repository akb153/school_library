 ...
 */
@WebServlet("/GraduationComplete")
public class GraduationComplete extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public GraduationComplete() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		//ログイン情報のDELETEフラグをオンにする（論理削除）
		LoginBean deleteUser = (LoginBean) session.getAttribute("delete_user");
		try {
			UserOperation.deleteStudent(deleteUser);
		} catch (SQLException | NamingException e) {
			// TODO 自動生成された catch ブロック
		}

		// graduation_complete.jspへ転送
		request.getRequestDispatcher("/WEB-INF/view/graduation_complete.jsp").forward(request, response);
