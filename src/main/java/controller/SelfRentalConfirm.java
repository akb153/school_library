 ...
 */
@WebServlet("/SelfRentalConfirm")
public class SelfRentalConfirm extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public SelfRentalConfirm() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if(exist_check_result == false) {
				request.getRequestDispatcher("/WEB-INF/view/self_rental.jsp?error=no_exist").forward(request, response);
			}
		...
		for(int i = 0; i < book_list.size(); i++) {
			for (int y = book_list.size()-1 ; i <  y ; y--) {
				if(book_list.get(i).toString().equals(book_list.get(y).toString())){
					request.getRequestDispatcher("/WEB-INF/view/self_rental.jsp?error=duplicate").forward(request, response);
			}
			}
		}
		...
		try {
			...
			if(overdue_check_result == true) {
				request.getRequestDispatcher("/WEB-INF/view/self_rental.jsp?error=overdue").forward(request, response);
			}
		} catch (SQLException | NamingException e1) {
			// TODO 自動生成された catch ブロック
			e1.printStackTrace();
		...
		try {
			...
			if(reservation_check_result.size() > 0) {
				...
				request.getRequestDispatcher("/WEB-INF/view/self_rental.jsp?error=reserved").forward(request, response);
			}
		} catch (SQLException | NamingException e1) {
			// TODO 自動生成された catch ブロック
			e1.printStackTrace();
		...
		try {
			...
			if(borrowing_check_result.size() > 0) {
				...
				request.getRequestDispatcher("/WEB-INF/view/self_rental.jsp?error=borrowed").forward(request, response);
			}
		} catch (SQLException | NamingException e1) {
			// TODO 自動生成された catch ブロック
			e1.printStackTrace();
		...
		try {
			int borrowing_num = RentalingBook.checkLimit(user_id);
				...
				request.getRequestDispatcher("/WEB-INF/view/self_rental.jsp?error=limit_over").forward(request, response);
				}
		} catch (SQLException | NamingException e1) {
			// TODO 自動生成された catch ブロック
			e1.printStackTrace();
		...
		request.getRequestDispatcher("/WEB-INF/view/self_rental_confirm.jsp").forward(request, response);
